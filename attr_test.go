...
func BenchmarkBadgerAttrStore_Duplicate(b *testing.B) {

	s := MustOpenBadgerAttrStore()
	defer s.Close()
...
func NewBadgerAttrStore(string) pilosa.AttrStore {
	f, err := ioutil.TempFile("", "pilosa-attr-")
	if err != nil {
		panic(err)
	}
	...
	return &AttrStore{badger.NewAttrStore(f.Name())}
}
func MustOpenBadgerAttrStore() pilosa.AttrStore {
	s := NewBadgerAttrStore("")
	if err := s.Open(); err != nil {
		panic(err)
	}
	return s
