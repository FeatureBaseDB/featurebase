...
func (f *Field) importValue(qcx *Qcx, columnIDs []uint64, values []int64, shard uint64, options *ImportOptions) (err0 error) {
	...
	for i := range columnIDs {
		...
		if value > bsig.Max {
			return fmt.Errorf("%v, value=%v, columnID=%v", ErrBSIGroupValueTooHigh, value, columnID)
		} else if value < bsig.Min {
			return fmt.Errorf("%v, value=%v, columnID=%v", ErrBSIGroupValueTooLow, value, columnID)
		}
		if value > max {
			max = value
		}
