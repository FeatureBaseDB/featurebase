...
// precomputed values. For instance, index operations, or operations
// which are flagged as needing precomputed values. (This would usually
// be used for things where the outputs don't necessarily have the same
// shards as their inputs.)
func (e *executor) handlePreCalls(ctx context.Context, index string, c *pql.Call, shards []uint64, opt *execOptions) error {
	if c.Name == "Precomputed" {
		...
		if idx >= 0 && idx < int64(len(opt.EmbeddedData)) {
			...
			return fmt.Errorf("no precomputed data! index %d, len %d\n", idx, len(opt.EmbeddedData))
		}
		return nil
	}
...
// executeGenericFieldShard runs a generic field function on a shard.
func (e *executor) executeGenericFieldShard(ctx context.Context, index string, c *pql.Call, op ext.BitmapOpBSIBitmap, shard uint64) (SignedRow, error) {
	span, ctx := tracing.StartSpanFromContext(ctx, "Executor.executeGenericFieldShard")
	defer span.Finish()
	...
	if len(c.Children) == 1 {
		...
		if filter != nil && len(filter.segments) > 0 {
			...
			// we *had* a filter, it's just empty here.
